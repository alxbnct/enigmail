...
var EnigmailOpenPGP = ChromeUtils.import("chrome://enigmail/content/modules/openpgp.jsm").EnigmailOpenPGP;
...
Enigmail.hdrView = {
    ...
    if (!(statusFlags & EnigmailConstants.DECRYPTION_FAILED) &&
      ...
      if (sigDetails) {
        ...
        if (detailArr.length > 7) {
          var signingAlg = EnigmailOpenPGP.signingAlgIdToString(detailArr[6]);
          var hashAlg = EnigmailOpenPGP.hashAlgIdToString(detailArr[7]);
  ...
  headerPane: {
    ...
    updateSecurityStatus: function(unusedUriSpec, exitCode, statusFlags, keyId, userId, sigDetails, errorMsg, blockSeparation, uri, extraDetails, mimePartNumber) {
      // uriSpec is not used for Enigmail anymore. It is here becaue other addons rely on it
