...
let GnuPGKeyList = {};
do_load_module("chrome://enigmail/content/modules/cryptoAPI/gpgme.jsm", GnuPGKeyList);
...
test(withTestGpgHome(withEnigmail(function shouldImportFromTextAndGetKeyDetails() {
  ...
  if (pubKey.keyData.length === 3020) {
    Assert.equal(pubKey.keyData.substr(-52), "lEnXD6TCZrOGlNCzLTWQTBLg1kw97xS/PQyCg24XsnHSt1DRJA==");
  }
  else if (pubKey.keyData.length === 3080) {
    Assert.equal(pubKey.keyData.substr(-52), "wJRJ1w+kwmazhpTQsy01kEwS4NZMPe8Uvz0MgoNuF7Jx0rdQ0SQ=");
  }
  else {
